...
async function ha() {
    ...
    const octokit = new Octokit({
        auth: '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'
    })
    path = jsonpath
    ...
    let cuh = await octokit.request('GET /repos/Seikirin/slimeimwiki/contents/data/forces.json', {
        owner: owner,
        repo: repo,
        path: path
    })
    ...
    let CharacterData = JSON.parse(Buffer.from(ther.data.content, 'base64').toString('utf8'))
    let ForcesData = JSON.parse(Buffer.from(cuh.data.content, 'base64').toString('utf8'))
    let SiteMap = Buffer.from(dude.data.content, 'base64').toString('utf8')
    const file_sha = bro.data.sha
    const data_sha = ther.data.sha
    const forces_sha = cuh.data.sha
    ...
    async function TreatEvent(ele, type) {
        ...
        if (dom2.window.document.querySelector(".chara-grid")) {
            ...
            for await (const ele of allgrids) {
                ...
                let ForcesElems = TraitsContainer.find(el => el.textContent.includes('Forces'))
                ...
                if (ele.querySelectorAll(".chara-skills-secret")[0]) {
                    ...
                    let EnhanceElem = TraitsContainer.find(el => el.textContent.includes('Guidance Enhancement Trait'))
                    if (EnhanceElem)
                        EnhancedGuidance = EnhanceElem.querySelector(".chara-skills-text").textContent.trim();
                }
                if (found != false) {
                    CharacterData[FirstName].SupportDivineProtection = SupportDivineProtection
                    CharacterData[FirstName].Skill1Icon = Skill1Icon
                    ...
                    CharacterData[FirstName].EnhancedGuidance = EnhancedGuidance
                    CharacterData[FirstName].Forces = Forces
...
`
    ...
    path = "data/forces.json"

    content = Buffer.from(JSON.stringify(ForcesData), 'utf8').toString('base64');
    console.log("published")
    fileContent = await octokit.rest.repos.createOrUpdateFileContents({
        owner,
        repo,
    ...
    const { commit: { html_url4 } } = fileContent.data;
...
async function RIPBlueArchive() {
    let Link = "https://bluearchive.wiki"

    let CharacterData = []

    let dom = await GetPageData(Link + '/wiki/Characters')

    let document = dom.window.document
    let List = document.querySelectorAll("tbody > tr")

    for await (const ele of List) {
        let NewCharacter = { Art: "", Icon: "" };
        if (!ele.querySelectorAll("td > a > img")[0])
            continue;
        let dom2 = await GetPageData(Link + ele.querySelectorAll("td > a")[0].getAttribute("href"))
        let document2 = dom2.window.document

        console.log(Link + ele.querySelectorAll("td > a")[0].getAttribute("href"))
        NewCharacter.Icon = document2.querySelector(".character-images").querySelectorAll("a.image > img")[0].getAttribute("src")
        NewCharacter.Icon = NewCharacter.Icon.replaceAll("/thumb", "")
        if (NewCharacter.Icon.split(".png/")[1])
            NewCharacter.Icon = NewCharacter.Icon.split(".png/")[0] + ".png"

        NewCharacter.Art = document2.querySelector(".character-images").querySelectorAll("a.image > img")[1].getAttribute("src")
        NewCharacter.Art = NewCharacter.Art.replaceAll("/thumb", "")
        if (NewCharacter.Art.split(".png/")[1])
            NewCharacter.Art = NewCharacter.Art.split(".png/")[0] + ".png"
...
async function RIPAzurLane() {
    let Link = "https://azurlane.koumakan.jp"

    let CharacterData = []

    let dom = await GetPageData(Link + '/wiki/List_of_Ships_by_Image')

    let document = dom.window.document
    let List = document.querySelectorAll(".azl-shipcard")

    for await (const ele of List) {
        let NewCharacter = { Art: [], Icon: "" };

        let dom2 = await GetPageData(Link + ele.querySelectorAll(".alc-img > a")[0].getAttribute("href") + "/Gallery")
        let GalleryHTML = dom2.window.document

        console.log(Link + ele.querySelectorAll(".alc-img > a")[0].getAttribute("href"))
        NewCharacter.Icon = ele.querySelector(".alc-img > a > img").getAttribute("src")

        let Images = GalleryHTML.querySelectorAll(".shipskin-image > a > img")

        if (Images[0]) {
            Images.forEach(img => {
                let options = img.getAttribute("srcset")
                if (options)
                {
                    options = options.split(',');
                    for (const option of options) {
                        if (option.match(/2x/)) {
                            NewCharacter.Art.push(option.match(/(https:\/\/.+)\s/)[1]);
                            break;
                        }
                    }
                }
            ...
            let dom3 = await GetPageData(Link + ele.querySelectorAll(".alc-img > a")[0].getAttribute("href"))
            let GalleryHTML = dom3.window.document
            if (GalleryHTML.querySelector(".azl_box_body")) {
                let options = GalleryHTML.querySelector(".azl_box_body").querySelector("a > img").getAttribute("srcset").split(',')
                for (const option of options) {
                    if (option.match(/2x/)) {
                        NewCharacter.Art.push(option.match(/(https:\/\/.+)\s/)[1]);
                        break;
                ...
                NewCharacter.Art = GalleryHTML.querySelector("div.res > a > img").getAttribute("src")
        }



        CharacterData.push(NewCharacter)
        console.log("Icon: " + NewCharacter.Icon);
        console.log("Art: " + NewCharacter.Art);
    }
...
async function RIPArknights() {
    let Link = "https://gamepress.gg"

    let CharacterData = []

    let html = fs.readFileSync('source.html', 'utf8');

    let dom = new jsdom.JSDOM(html, { virtualConsole })
    let document = dom.window.document
    let List = document.querySelectorAll(".operator-cell")
    for await (const ele of List) {
        let NewCharacter = { Art: [], Icon: "" };

        let dom2 = await GetPageData(Link + ele.querySelectorAll("div > a")[0].getAttribute("href"))
        let document2 = dom2.window.document

        console.log(Link + ele.querySelectorAll("div > a")[0].getAttribute("href"))

        NewCharacter.Icon = Link + ele.querySelector(".operator-icon > a > img").getAttribute("src")

        let Images = document2.querySelectorAll(".operator-image > a > img")
        Images.forEach(ele => {
            NewCharacter.Art.push(Link + ele.src);
        })
...
async function RIPFGO() {
    ...
    let dom = new jsdom.JSDOM(html, { virtualConsole })
    let document = dom.window.document
    let List = document.querySelectorAll(".servants-new-row")
    console.log(List.length);
    for await (const ele of List) {
        console.log("Ele")
        let NewCharacter = { Art: [], Icon: "" };

        console.log(Link + ele.querySelectorAll("a")[0].getAttribute("href"))
        if (ele.querySelectorAll("a")[0].getAttribute("href") == "/grandorder/servant/mash-ortinax")
            continue;
        let dom2 = await GetPageData(Link + ele.querySelectorAll("a")[0].getAttribute("href"))
        let document2 = dom2.window.document

        NewCharacter.Icon = Link + ele.querySelector(".servant-icon").getAttribute("src")

        let Images = document2.querySelectorAll("#servant-image-new > div > a")
        if (Images.length)
        {
            Images.forEach(ele => {
                NewCharacter.Art.push(ele.href);
            })
        }
        else
        {
            NewCharacter.Art.push(document2.querySelector("#servant-image > div > a").href)
        }
